...
import com.felan.photoeditor.utils.BuildVars;
...
public class TelegramEGLThread extends DispatchQueue {
    ...
    public TelegramEGLThread(SurfaceTexture surface, Bitmap bitmap, FilterableImageView boundImg) {
        super("EGLThread");
        surfaceTexture = surface;
        currentBitmap = bitmap;
        this.boundImg = boundImg;
    }
    ...
    private void drawCustomParamsPass() {
        ...
        if (showOriginal) {
            ...
            int tintHighlightsColor = getTintHighlightsColor();
            ...
            int tintShadowsColor = getTintShadowsColor();
            ...
            boolean skipTone = getCurveToolValue().shouldBeSkipped();
            GLES20.glUniform1f(skipToneHandle, skipTone ? 1.0f : 0.0f);
            if (!skipTone) {
                getCurveToolValue().fillBuffer();
                GLES20.glActiveTexture(GLES20.GL_TEXTURE1);
                ...
                GLES20.glTexParameteri(GL10.GL_TEXTURE_2D, GL10.GL_TEXTURE_WRAP_S, GL10.GL_CLAMP_TO_EDGE);
                GLES20.glTexParameteri(GL10.GL_TEXTURE_2D, GL10.GL_TEXTURE_WRAP_T, GL10.GL_CLAMP_TO_EDGE);
                GLES20.glTexImage2D(GLES20.GL_TEXTURE_2D, 0, GLES20.GL_RGBA, 200, 1, 0, GLES20.GL_RGBA, GLES20.GL_UNSIGNED_BYTE, getCurveToolValue().getCurveBuffer());
    ...
    private boolean drawBlurPass() {
        ...
        if (getBlurType() == 1) {
            ...
            GLES20.glUniform1f(radialBlurExcludeSizeHandle, getBlurExcludeSize());
            GLES20.glUniform1f(radialBlurExcludeBlurSizeHandle, getBlurExcludeBlurSize());
            GLES20.glUniform2f(radialBlurExcludePointHandle, getBlurExcludePoint().x, getBlurExcludePoint().y);
            ...
            GLES20.glUniform1f(linearBlurExcludeSizeHandle, getBlurExcludeSize());
            GLES20.glUniform1f(linearBlurExcludeBlurSizeHandle, getBlurExcludeBlurSize());
            GLES20.glUniform1f(linearBlurAngleHandle, getBlurAngle());
            GLES20.glUniform2f(linearBlurExcludePointHandle, getBlurExcludePoint().x, getBlurExcludePoint().y);
    ...
    private FilterableImageView boundImg;

    public void setBoundImg(FilterableImageView boundImg) {
        this.boundImg = boundImg;
    }

    private float getShadowsValue() {
        return (boundImg.getShadowsValue() * 0.55f + 100.0f) / 100.0f;
    }

    private float getHighlightsValue() {
        return (boundImg.getHighlightsValue() * 0.75f + 100.0f) / 100.0f;
    }

    private float getEnhanceValue() {
        return (boundImg.getEnhanceValue() / 100.0f);
    }

    private float getExposureValue() {
        return (boundImg.getExposureValue() / 100.0f);
    }

    private float getContrastValue() {
        return (boundImg.getContrastValue() / 100.0f) * 0.3f + 1;
    }

    private float getWarmthValue() {
        return boundImg.getWarmthValue() / 100.0f;
    }

    private float getVignetteValue() {
        return boundImg.getVignetteValue() / 100.0f;
    }

    private float getSharpenValue() {
        return 0.11f + boundImg.getSharpenValue() / 100.0f * 0.6f;
    }

    private float getGrainValue() {
        return boundImg.getGrainValue() / 100.0f * 0.04f;
    }

    private float getFadeValue() {
        return boundImg.getFadeValue() / 100.0f;
    }

    private float getTintHighlightsIntensityValue() {
        float tintHighlightsIntensity = 50.0f;
        return boundImg.getTintHighlightsColor() == 0 ? 0 : tintHighlightsIntensity / 100.0f;
    }

    private int getTintHighlightsColor() {
        return boundImg.getTintHighlightsColor();
    }

    private float getTintShadowsIntensityValue() {
        float tintShadowsIntensity = 50.0f;
        return boundImg.getTintShadowsColor() == 0 ? 0 : tintShadowsIntensity / 100.0f;
    }

    private int getTintShadowsColor() {
        return boundImg.getTintShadowsColor();
    }

    private float getSaturationValue() {
        float parameterValue = (boundImg.getSaturationValue() / 100.0f);
        if (parameterValue > 0) {
    ...
    private int getBlurType() {
        return boundImg.getBlurType().getValue();
    }

    private float getBlurExcludeSize() {
        return boundImg.getBlurExcludeSize();
    }

    private PointF getBlurExcludePoint() {
        return boundImg.getBlurExcludePoint();
    }

    private float getBlurExcludeBlurSize() {
        return boundImg.getBlurExcludeBlurSize();
    }

    private float getBlurAngle() {
        return boundImg.getBlurAngle();
    }

    private CurvesToolValue getCurveToolValue() {
        return boundImg.getCurvesToolValue();
    }
}
